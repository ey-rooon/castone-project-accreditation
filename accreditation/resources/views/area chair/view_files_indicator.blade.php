<x-app-layout>
    ...
    @endif
    <div class="p-6">
        <a href="/view_indicator_areachair/{{ $parameter_id }}/{{ $accreditation_id }}">
            <button class="btn btn-outline-secondary">Go Back</button>
        </a>
        <div class="row p-2">
            <h4>Files of {{ $indicator->indicator_name }}: {{ $indicator->indicator_desc }}</h4>
        </div>
        @php
            $areaid = $parameter->where('id', $indicator->parameter_id)->value('area_id');
            $area = $area_member;
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form enctype="multipart/form-data"
                                                        action="/updateindicatorfile/{{ $file->id }} "
                                                        method="post">
                                                        @csrf
                                                        <div class="mb-3">
                                                ...
                                                @if (Auth::User()->current_role == 'area member' || Auth::User()->current_role == 'chair')
                                                    <div class="col-12 col-md-3 border rounder overflow-y">
                                                        ...
                                                        <div class="row-12 row-col-10" style="height: 80%;">
                                                            @php
                                                                $msg = $messages->where('indicator_file_id', $file->id);
                                                            @endphp
                                                        ...
                                                        <div class="row-12 row-col-2 d-flex align-items-center"
                                                            style="height: 20%;">
                                                            <form method="post" action="/send_message_indicator"
                                                                class="message-form">
                                                                @csrf
